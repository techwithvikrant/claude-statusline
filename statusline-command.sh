#!/usr/bin/env bash
# Claude Code status line. Reads the session JSON on stdin and renders two lines:
#   <model> │ ctx [bar] N% · used/size │ 5h [bar] N% ⟳ <reset IST> │ 7d [bar] N% · Nd Nh
#   <directory>  <git-branch>
# Proactive markers prefix any metric that's getting tight: ▲ caution (>=75%),
# ⚠ critical (>=90%), ↻ when a window resets within 10 minutes.
# Bars use Unicode partial blocks for smooth sub-character fill. All values come
# from Claude Code's native status-line JSON (v2.x: context_window.* and
# rate_limits.five_hour/seven_day.*), recomputed every render — works in every
# session with no transcript parsing.
...
def _fmt_clock(dt):
    # 12-hour clock without leading zero; %-I is unsupported on some libcs.
    try:
...
CACHE = os.path.expanduser("~/.claude/.statusline-cache.json")
try:
